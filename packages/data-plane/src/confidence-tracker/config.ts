...
import type { ConfidenceTrackerConfig } from './confidence-tracker';
import type { InfluxDBStoreConfig } from './influxdb-store';
...
export function getConfidenceTrackerConfig(): ConfidenceTrackerConfig {
  ...
  const baseConfig: ConfidenceTrackerConfig = {
    maxDataPoints: parseInt(
      process.env.PARALLAX_MAX_DATA_POINTS || '100000',
      10
    ),
    retentionPeriodDays: parseInt(
      process.env.PARALLAX_RETENTION_DAYS || '7',
      10
    ),
    ...
    anomalyDetection: {
      ...
      suddenDropThreshold: parseFloat(
        process.env.PARALLAX_SUDDEN_DROP_THRESHOLD || '0.3'
      ),
      lowConfidenceThreshold: parseFloat(
        process.env.PARALLAX_LOW_CONFIDENCE_THRESHOLD || '0.5'
      ),
      highVarianceThreshold: parseFloat(
        process.env.PARALLAX_HIGH_VARIANCE_THRESHOLD || '0.2'
      ),
      checkIntervalMs: parseInt(
        process.env.PARALLAX_ANOMALY_CHECK_INTERVAL || '60000',
        10
      ),
    },
  ...
  if (store === 'influxdb') {
    const influxConfig: InfluxDBStoreConfig = {
      ...
      retentionPeriodDays: parseInt(
        process.env.INFLUXDB_RETENTION_DAYS || '30',
        10
      ),
    };
