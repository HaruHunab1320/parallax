...
export class PatternAssembler {
  ...
  async assemble(pattern: ComposedPattern): Promise<ExecutablePattern> {
    ...
    if (!validation.isValid) {
      throw new Error(
        `Generated pattern is invalid: ${validation.errors.join(', ')}`
      );
    }
...
`;
  ...
  private generateImports(pattern: ComposedPattern): string {
    ...
    for (const [category, primitives] of primitivesByCategory) {
      ...
      imports.push(
        `import { ${primitiveList} } from "@parallaxai/primitives/${category}"`
      );
    }
  ...
  private generateMainFunction(pattern: ComposedPattern): string {
    ...
    const functionName = pattern.name.replace(/-([a-z])/g, (_, letter) =>
      letter.toUpperCase()
    );
    ...
    lines.push(
      `    minConfidence = config.minConfidence || ${pattern.metadata.minConfidence || 0.5}`
    );
    lines.push(
      `    strategy = config.strategy || "${pattern.metadata.strategy || 'default'}"`
    );
  ...
  private generatePrimitiveCode(
    ...
    switch (primitive.name) {
      case 'parallel':
        lines.push(
          `  ${varName} = parallel(${primitive.config?.maxConcurrency || 'null'})(agents.map(agent => {`
        );
      ...
      case 'retry':
        lines.push(
          `  ${varName} = retry(maxRetries, "exponential")(${inputVar})`
        );
        break;

      case 'fallback':
        lines.push(
          `  ${varName} = fallback(config.fallback || "default")(${inputVar})`
        );
        break;

      case 'escalate':
        lines.push(
          `  ${varName} = escalate(config.escalationPath || "supervisor")(${inputVar})`
        );
        break;
  ...
  private generateWrapperCode(
    ...
    switch (wrapper.name) {
      case 'retry':
        ...
        lines.push(
          `  finalResult = retry(maxRetries, "exponential")(() => ${wrappedVar})`
        );
      ...
      case 'timeout':
        ...
        lines.push(
          `  finalResult = timeout(config.timeout || 30000)(() => ${wrappedVar})`
        );
        variableMap.set('final', 'finalResult');
        break;
    }
