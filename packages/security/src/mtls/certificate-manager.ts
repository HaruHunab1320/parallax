...
export class CertificateManager {
  ...
  private async generateCA(config: CertificateConfig): Promise<void> {
    ...
    const attrs = [
      ...
      { name: 'organizationName', value: config.organization || 'Parallax' },
      {
        ...
        value: config.organizationalUnit || 'Platform',
      },
    ];
  ...
  async generateCertificate(
    ...
    const attrs = [
      ...
      {
        name: 'organizationalUnitName',
        value: config.organizationalUnit || 'Agent',
      },
    ];
  ...
  async saveCertificateSet(
    ...
    await fs.writeFile(path.join(certDir, 'key.pem'), certSet.privateKey, {
      mode: 0o600,
      encoding: 'utf8',
    });
  ...
  async loadCertificateSet(name: string): Promise<CertificateSet> {
    ...
    const privateKey = await fs.readFile(path.join(certDir, 'key.pem'), 'utf8');

    let caCertificate: string | undefined;
    try {
      caCertificate = await fs.readFile(path.join(certDir, 'ca.pem'), 'utf8');
    } catch (_error) {
  ...
  private async loadCA(): Promise<void> {
    ...
    const certPem = await fs.readFile(path.join(caPath, 'cert.pem'), 'utf8');

    const keyPem = await fs.readFile(path.join(caPath, 'key.pem'), 'utf8');
