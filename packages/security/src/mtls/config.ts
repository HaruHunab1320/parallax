...
import type { MTLSConfig } from './credentials-provider';
...
export function getMTLSConfig(): MTLSConfig {
  ...
  return {
    enabled: true,
    certsDir:
      process.env.PARALLAX_CERTS_DIR || path.join(process.cwd(), 'certs'),
...
export function getServiceMTLSConfig(serviceName: string): MTLSConfig {
  ...
  return {
    ...
    checkClientCertificate:
      process.env[`${servicePrefix}VERIFY_CLIENT`] === 'true' ||
      baseConfig.checkClientCertificate,
  };
}
