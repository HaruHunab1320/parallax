...
async function main() {
  const certsDir =
    process.env.PARALLAX_CERTS_DIR || path.join(__dirname, '../../certs');
  ...
  for (const service of services) {
    console.log(
      `  ${service.name}: ${path.join(certsDir, service.name, 'cert.pem')}`
    );
  }
  console.log('\nAgent Certificates:');
  for (const agent of agents) {
    console.log(
      `  ${agent.name}: ${path.join(certsDir, agent.name, 'cert.pem')}`
    );
  }
  console.log('\nClient Certificates:');
  for (const client of clients) {
    console.log(
      `  ${client.name}: ${path.join(certsDir, client.name, 'cert.pem')}`
    );
  }
  console.log('\n=========================\n');
}
