...
export class ConfidenceProtocol {
  shouldExploreParallel(results: AgentResult<any>[]): boolean {
    const highConfidenceResults = results.filter(
      (r) => r.confidence >= DEFAULT_THRESHOLDS.high
    );
  ...
  calculateWeightedConsensus(results: AgentResult<any>[]): {
    consensus: number;
...
export class EpistemicOrchestrator implements CoordinationPattern {
  ...
  description =
    'Multiple agents analyze from different perspectives, high-confidence disagreements trigger parallel paths';
  ...
  async execute<T>(agents: Agent[], task: string, data?: any): Promise<T> {
    ...
    const highestConfidence = results.reduce(
      (prev: AgentResult<T>, current: AgentResult<T>) =>
  ...
  private createParallelRecommendation<T>(results: AgentResult<T>[]) {
    const { consensus, disagreements } =
      this.protocol.calculateWeightedConsensus(results);
    ...
    return {
      ...
      disagreements,
      recommendation:
        'High-confidence disagreements detected. Consider exploring parallel paths.',
    };
  }
}
