      ...
      stages: [
        {
          ...
          parallel: true,
          agents: [{ capability: 'static-analyzer', count: 3 }],
        },
        {
          name: 'dependency-check',
          agents: [{ capability: 'dependency-scanner', count: 2 }],
        },
        {
          name: 'penetration-test',
          agents: [{ capability: 'pen-tester', count: 1 }],
        },
      ],
      ...
      stages: [
        {
          name: 'pattern-match',
          parallel: true,
          agents: [{ capability: 'pattern-detector', count: 5 }],
        },
        {
          name: 'ml-analysis',
          agents: [{ capability: 'ml-analyzer', count: 3 }],
        },
        {
          name: 'decision',
          agents: [{ capability: 'fraud-expert', count: 1 }],
        },
      ],
      constraints: {
...
class BattleTestRunner {
  ...
  async runAllTests(): Promise<void> {
    ...
    console.log(
      `Running ${TEST_CASES.length} test cases across ${this.getCategories().length} categories\n`
    );
  ...
  analyzePattern(pattern: any, validation: any): TestResult['analysis'] {
    ...
    const hasConfidenceExtraction =
      code.includes('.confidence') || code.includes('confidence:');
    const usesConfidenceOperator = code.includes('~>');
    const returnsWithConfidence =
      code.includes('return') && code.includes('~>');

    analysis.hasProperConfidenceFlow =
      hasConfidenceExtraction &&
      usesConfidenceOperator &&
      returnsWithConfidence;
    ...
    if (
      !code.includes('map(') &&
      pattern.metadata?.primitives?.includes('parallel')
    ) {
      analysis.issues.push(
        'Parallel primitive used but no agent mapping found'
      );
    }

    if (code.includes('step1 = input') && code.includes('step2 = step1')) {
      analysis.issues.push(
        "No-op steps detected (steps that don't transform data)"
      );
    }
    ...
    topIssues.forEach(({ issue }) => {
      if (issue.includes('Hardcoded confidence')) {
        recommendations.push(
          'Improve confidence extraction logic to avoid hardcoded values'
        );
      }
      if (issue.includes('No-op steps')) {
        recommendations.push(
          'Optimize step generation to avoid redundant transformations'
        );
      }
      if (issue.includes('Validation error')) {
        recommendations.push(
          'Enhance Prism syntax generation to reduce validation errors'
        );
      }
    });
    ...
    Object.entries(categoryBreakdown).forEach(
      ([category, stats]: [string, any]) => {
        if (stats.successRate < 50) {
          recommendations.push(
            `Improve handling of ${category} patterns (${stats.successRate}% success rate)`
          );
        }
      }
    );
  ...
  private generateMarkdownReport(report: any): string {
    ...
    md += '|----------|-------|---------|--------|-------------|\n';
    Object.entries(report.byCategory).forEach(
      ([category, stats]: [string, any]) => {
  ...
  private calculateAverageComplexity(): number {
    const complexities = this.results
  ...
  private calculateValidSyntaxRate(): number {
    const validSyntax = this.results.filter(
      (r) => r.analysis.hasValidSyntax
    ).length;
    return Math.round((validSyntax / this.results.length) * 100);
  }

  private calculateProperConfidenceFlowRate(): number {
    const properFlow = this.results.filter(
      (r) => r.analysis.hasProperConfidenceFlow
    ).length;
...
export { BattleTestRunner, TEST_CASES, type TestCase, type TestResult };
