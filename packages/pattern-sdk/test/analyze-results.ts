...
class ResultAnalyzer {
  ...
  async analyze(): Promise<void> {
    ...
    if (!(await fs.pathExists(this.reportPath))) {
      console.log(
        '❌ No test results found. Run tests first with --mini or --full'
      );
      return;
    }
  ...
  private async analyzePatternQuality(): Promise<any> {
    ...
    patterns.forEach(({ content, filename }) => {
      ...
      if (
        ...
        content.includes('from "@parallaxai/primitives')
      ) {
      ...
      if (
        content.includes('finalConfidence = result.confidence') &&
    ...
    if (patterns.length > 0) {
      quality.averageLineCount = Math.round(
        quality.averageLineCount / patterns.length
      );
    }
  ...
  private async loadGeneratedPatterns(): Promise<
  ...
  private generateAnalysisMarkdown(analysis: any): string {
    ...
    const criticalInsights = analysis.insights.filter(
      (i: AnalysisInsight) => i.severity === 'critical'
    );
    const highInsights = analysis.insights.filter(
      (i: AnalysisInsight) => i.severity === 'high'
    );
  ...
  private printKeyFindings(
    ...
    console.log(
      `- Proper Confidence Flow: ${report.summary.properConfidenceFlowRate}%`
    );
    ...
    const categories = Object.entries(report.byCategory).sort(
      (a: any, b: any) => b[1].successRate - a[1].successRate
    );
    if (categories.length > 0) {
      ...
      console.log(
        `❌ Worst performing: ${categories[categories.length - 1][0]} (${categories[categories.length - 1][1].successRate}% success)`
      );
    }
