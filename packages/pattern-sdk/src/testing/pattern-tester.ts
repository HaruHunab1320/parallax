...
export class PatternTester {
  ...
  async test(options: TestOptions): Promise<TestResult> {
    ...
    try {
      ...
      if (
        options.expectedConfidence &&
        result.confidence < options.expectedConfidence
      ) {
        errors.push(
          `Confidence ${result.confidence} below expected ${options.expectedConfidence}`
        );
      }
  ...
  private simulateConsensus(
    results: Array<{ result: any; confidence: number }>
  ) {
    ...
    const avgConfidence =
      consensusConfidences.reduce((sum, c) => sum + c, 0) /
      consensusConfidences.length;
