...
import type { OrchestrationRequirements, PrimitiveDefinition } from '../types';
...
export class PatternAssembler {
  ...
  async assemble(
    ...
    const patternLogic = this.generatePatternLogic(
      requirements,
      selectedPrimitives,
      order
    );
    const indentedLogic = patternLogic
      .split('\n')
      .map((line) => (line ? `  ${line}` : ''))
      .join('\n');
  ...
  private generateJSDocHeader(requirements: OrchestrationRequirements): string {
    ...
    if (requirements.agents && requirements.agents.length > 0) {
      const capabilities = requirements.agents
        .map((a) => a.capability)
        .filter(Boolean);
  ...
  private calculateMinAgents(requirements: OrchestrationRequirements): number {
    if (requirements.agents && requirements.agents.length > 0) {
      return requirements.agents.reduce(
        (sum, agent) => sum + (agent.count || 1),
        0
      );
    }
  ...
  private generatePatternLogic(
    ...
    const primaryPrimitive =
      selectedPrimitives[0]?.name || requirements.strategy;
    ...
    switch (primaryPrimitive) {
      ...
      case 'fallback':
        lines.push(
          ...this.generateRobustPattern(requirements, selectedPrimitives)
        );
        break;
      case 'threshold':
      ...
      default:
        lines.push(
          ...this.generateGenericPattern(requirements, selectedPrimitives)
        );
    }
   ...
   */
  private generateConsensusPattern(
    requirements: OrchestrationRequirements
  ): string[] {
    const lines: string[] = [];
    ...
    lines.push('result = {');
    lines.push(
      '  status: consensusReached ? "consensus_reached" : "low_consensus",'
    );
  ...
  private generateRobustPattern(
    ...
    lines.push('result = {');
    lines.push(
      '  strategy: needsRecovery ? "recovery_executed" : "first_attempt_succeeded",'
    );
   ...
   */
  private generateThresholdPattern(
    requirements: OrchestrationRequirements
  ): string[] {
    const lines: string[] = [];
   ...
   */
  private generateGenericPattern(
    requirements: OrchestrationRequirements,
    _primitives: any[]
  ): string[] {
    const lines: string[] = [];
