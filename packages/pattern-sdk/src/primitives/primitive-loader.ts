...
export class PrimitiveLoader {
  ...
  private extractMetadata(
    ...
    for (const line of lines) {
      ...
      if (trimmed.startsWith('// Primitive:')) {
        ...
        if (declaredCategory !== category) {
          console.warn(
            `Category mismatch in ${filePath}: declared '${declaredCategory}' but found in '${category}'`
          );
        }
    ...
    const mainFunctionRegex = new RegExp(
      `export\\s+const\\s+${metadata.name}\\s*=\\s*\\(([^)]*?)\\)`,
      's'
    );
  ...
  private getDefaultPrimitivesPath(): string {
    try {
      ...
      const primitivesPackage = require.resolve(
        '@parallaxai/primitives/package.json'
      );
  ...
  getPrimitivesByConfidence(confidenceStrategy: string): LoadedPrimitive[] {
    return Array.from(this.primitives.values()).filter(
      (primitive) => primitive.metadata.confidence === confidenceStrategy
    );
  }
...
export async function loadPrimitives(
  options?: PrimitiveLoaderOptions
): Promise<Map<string, LoadedPrimitive>> {
