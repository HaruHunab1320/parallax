...
export class PatternGeneratorWithPrimitives {
  ...
  generatePattern(requirements: {
    ...
    patterns.push(
      `// Generated pattern using ${requirements.strategy} execution`
    );
    patterns.push(
      `import { ${requirements.strategy} } from '@parallaxai/primitives/execution/${requirements.strategy}';`
    );
    ...
    if (requirements.needsRetry) {
      ...
      if (retryPrimitive) {
        patterns.push(
          `import { retry } from '@parallaxai/primitives/control/retry';`
        );
      }
    }
    ...
    if (requirements.needsConsensus) {
      ...
      if (consensusPrimitive) {
        patterns.push(
          `import { consensus } from '@parallaxai/primitives/aggregation/consensus';`
        );
      }
    }
    ...
    if (requirements.confidenceThreshold) {
      ...
      if (thresholdPrimitive) {
        patterns.push(
          `import { threshold } from '@parallaxai/primitives/confidence/threshold';`
        );
      }
    }
    ...
    if (requirements.needsRetry) {
      patterns.push(
        `  const withRetry = retry(3, { backoff: 'exponential' });`
      );
    }
    ...
    if (requirements.confidenceThreshold) {
      patterns.push(
        `  const withThreshold = threshold(${requirements.confidenceThreshold})(${requirements.needsConsensus ? 'withConsensus' : 'execution'});`
      );
    }

    patterns.push('');
    patterns.push(
      `  return ${requirements.confidenceThreshold ? 'withThreshold' : requirements.needsConsensus ? 'withConsensus' : 'execution'};`
    );
    patterns.push('};');
...
async function demonstrateIntegration() {
  ...
  if (validation.unknownPrimitives.length > 0) {
    console.log(
      `Unknown primitives: ${validation.unknownPrimitives.join(', ')}`
    );
  }
}
