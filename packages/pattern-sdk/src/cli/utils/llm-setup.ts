      ...
      if (prompt.includes('analyze')) {
        ...
        mockObject = {
          selected: [
            {
              name: 'parallel',
              reason: 'Execute agents concurrently',
              config: { maxConcurrency: 5 },
            },
            {
              name: 'consensus',
              reason: 'Reach agreement among agents',
              config: { threshold: 0.7 },
            },
            {
              name: 'threshold',
              reason: 'Ensure minimum confidence',
              config: { min: 0.8 },
            },
            {
              name: 'fallback',
              reason: 'Handle low confidence cases',
              config: { to: 'expert' },
            },
          ],
          ...
          confidence: 0.9,
          reasoning:
            'Parallel execution with consensus and confidence thresholding',
        };
      }
...
function createAnthropicProvider(_apiKey: string, _model: string): LLMProvider {
  return {
    async generateObject<T>({
      ...
      return createOpenAIProvider(_apiKey, _model).generateObject<T>({
        schema,
        prompt,
        system,
      });
...
function createCustomProvider(): LLMProvider {
  return {
    async generateObject<T>({}: {
      ...
      console.log(
        chalk.yellow('Note: Using custom provider - implement your own logic')
      );
