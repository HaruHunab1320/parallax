...
export class PatternValidator {
   ...
   */
  private validatePrismSyntax(code: string): {
    valid: boolean;
    errors: ValidationError[];
    warnings: ValidationWarning[];
  } {
    try {
      ...
      if (result.formattedErrors) {
        result.formattedErrors.forEach((err: any) => {
          if (
            !errors.some(
              (e) => e.line === err.line && e.message === err.message
            )
          ) {
            errors.push({
              line: err.line,
  ...
  private validateSemantics(
    ...
    if (
      code.includes('agents') &&
      !code.includes('agents =') &&
      !code.includes('agentList')
    ) {
      warnings.push({
    ...
    if (
      pattern.requirements.minConfidence &&
      !code.includes(pattern.requirements.minConfidence.toString())
    ) {
      warnings.push({
  ...
  private generateSuggestions(pattern: Pattern, suggestions: string[]) {
    ...
    if (!code.includes('timeout') && pattern.metadata.estimatedAgents > 5) {
      suggestions.push(
        'Consider adding timeout for operations with many agents'
      );
    }
    ...
    if (code.includes('escalate') && !code.includes('circuit')) {
      suggestions.push(
        'Consider circuit breaker pattern for external escalations'
      );
    }
  }
  ...
  async autoFix(pattern: Pattern): Promise<Pattern> {
      ...
      if (!importedPrimitives.includes(primitive)) {
        ...
        if (category) {
          const importRegex = new RegExp(
            `import\\s*{([^}]+)}\\s*from\\s*["']@parallaxai/primitives/${category}["']`
          );
    ...
    fixedCode = fixedCode.replace(
      /(\w+)\s*~>\s*(?![\d.])/g,
      '$1 ~> 0.5 // default confidence applied'
    );
    ...
    if (!fixedCode.includes('export')) {
      ...
      if (!fixedCode.includes('export function')) {
        ...
        fixedCode =
          fixedCode.slice(0, firstImportEnd) +
