...
import type { OrchestrationRequirements } from '../src/types';
      ...
      return {
        object: {
          selected: [
            {
              name: 'parallel',
              reason: 'Review by multiple agents concurrently',
              config: { maxConcurrency: 3 },
            },
            {
              name: 'consensus',
              reason: 'Reach agreement on review findings',
              config: { threshold: 0.8 },
            },
            {
              name: 'threshold',
              reason: 'Ensure high confidence in results',
              config: { min: 0.9 },
            },
            {
              name: 'retry',
              reason: 'Retry failed reviews',
              config: { maxAttempts: 2 },
            },
            {
              name: 'fallback',
              reason: 'Escalate to senior reviewer if needed',
              config: { to: 'senior-engineer' },
            },
          ],
          order: ['parallel', 'consensus', 'threshold', 'retry', 'fallback'],
          confidence: 0.95,
          reasoning:
            'Comprehensive code review pattern with quality safeguards',
        },
      };
    }
  },
    ...
    stages: [
      {
        ...
        parallel: true,
        agents: [{ capability: 'code-review', count: 3 }],
      },
      {
        name: 'security-check',
        description: 'Security-focused review',
        condition: 'hasSecurityConcerns',
        agents: [{ capability: 'security-review', count: 2 }],
      },
    ],
    constraints: {
