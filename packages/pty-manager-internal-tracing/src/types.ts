...
export interface PTYManagerConfig {
  ...
  /**
   * Optional internal tracing/capture configuration.
   * Disabled by default. Intended for internal adapter research workflows.
   */
  capture?: PTYCaptureConfig;
}

/**
 * Internal PTY capture configuration.
 */
export interface PTYCaptureConfig {
  /** Enable PTY state capture (default: false) */
  enabled?: boolean;

  /** Root directory for capture artifacts */
  ...
  /** Normalized text buffer tail size used for state matching */
...
/**
 * Emitted when capture classifier detects a state change.
 */
export interface InteractionStateChangedInfo {
  sessionId: string;
  state: string;
  ruleId?: string;
  confidence: number;
