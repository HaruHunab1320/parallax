 ...
 */
export function getPrimitiveDescriptor(name: string): PrimitiveDescriptor | undefined {
  return PRIMITIVE_DESCRIPTORS.find(p => p.name === name);
}
...
export function getPrimitivesByCategory(): Record<string, string[]> {
  ...
  PRIMITIVE_DESCRIPTORS.forEach(primitive => {
 ...
 */
export function findCompatiblePrimitives(primitiveName: string): string[] {
 ...
 */
export function arePrimitivesCompatible(primitive1: string, primitive2: string): boolean {
  const desc1 = getPrimitiveDescriptor(primitive1);
  const desc2 = getPrimitiveDescriptor(primitive2);
  
  if (!desc1 || !desc2) return true; // Assume compatible if unknown
