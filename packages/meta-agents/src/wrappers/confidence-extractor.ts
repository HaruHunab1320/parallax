...
export interface ConfidenceExtractorOptions {
  defaultConfidence?: number;
  extractionStrategy?: 'llm' | 'keywords' | 'hybrid';
}

/**
 * Decorator for automatic confidence extraction
 */
export function withConfidence(options: ConfidenceExtractorOptions = {}) {
 ...
 */
function extractConfidenceFromLLM(result: any): number {
  if (!result) return 0.5;

  // Check for explicit confidence in response
  if (typeof result === 'object') {
    if ('confidence' in result) return normalizeConfidence(result.confidence);
    if ('_confidence' in result) return normalizeConfidence(result._confidence);
    if ('score' in result) return normalizeConfidence(result.score);
    if ('certainty' in result) return normalizeConfidence(result.certainty);
  }
 ...
 */
function extractConfidenceFromKeywords(result: any): number {
...
export class ConfidenceAggregator {
   ...
   */
  static combine(confidences: number[], strategy: 'min' | 'max' | 'avg' | 'weighted' = 'avg'): number {
   ...
   */
  static fromConsistency(results: any[]): number {
