...
async function runE2ETest() {
  ...
  const composer = createPatternComposerAgent(new MockLLMProvider(), {
    id: 'test-composer',
  ...
  console.log('- Selected Primitives:', compositionResult.metadata.selectedPrimitives.map((p: any) => p.name));
  ...
  const patternAwareAgent = makePatternAware(reviewAgent, {
    autoCompose: true,
    cachePatterns: true
  });
