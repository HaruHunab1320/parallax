...
export class PatternTracer {
  ...
  traceConfidenceAggregation(
    ...
    if (span) {
      span.addEvent('confidence.aggregation', {
        ...
        'parallax.aggregation.min_confidence': Math.min(
          ...inputs.map((i) => i.confidence)
        ),
        'parallax.aggregation.max_confidence': Math.max(
          ...inputs.map((i) => i.confidence)
        ),
   ...
   */
  createPatternContext(patternName: string, executionId: string): api.Context {
  ...
  addPatternMetadata(metadata: {
    ...
    if (span) {
      ...
      if (metadata.confidenceThreshold !== undefined) {
        attributes['parallax.pattern.confidence_threshold'] =
