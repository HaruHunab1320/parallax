...
import { PTYSession, SPECIAL_KEYS } from '../src/pty-session';
...
describe('normalizeKeyList', () => {
  it('should pass through already-normalized keys', () => {
    ...
    expect(PTYSession.normalizeKeyList(['up', 'up', 'enter'])).toEqual([
      'up',
      'up',
      'enter',
    ]);
  ...
  it('should handle mixed normalized and non-normalized keys', () => {
    expect(PTYSession.normalizeKeyList(['down', 'control', 'c'])).toEqual([
      'down',
      'ctrl+c',
    ]);
    expect(PTYSession.normalizeKeyList(['control', 'c', 'enter'])).toEqual([
      'ctrl+c',
      'enter',
    ]);
  });
