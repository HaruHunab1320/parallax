...
import type { Pattern } from '../../pattern-engine/types';
...
function valueToJs(value: any): any {
  ...
  if ('structValue' in value || 'struct_value' in value)
    return structToObject(value.structValue ?? value.struct_value);
...
export class PatternServiceImpl {
  ...
  async executePattern(
    ...
    try {
      const { pattern_name, input, input_json, parameters, options } =
        call.request;

      this.logger.info({ pattern: pattern_name }, 'Executing pattern via gRPC');

      const parsedInput = input_json
        ? JSON.parse(input_json)
        : structToObject(input);
  ...
  async streamExecutePattern(call: grpc.ServerWritableStream<any, any>) {
    try {
      ...
      this.logger.info(
        ...
        'Stream executing pattern via gRPC'
      );
  ...
  private toExecuteResponse(result: any, patternName: string): any {
    ...
    return {
      ...
      metrics: {
        ...
        agents_used:
          result.metrics?.agentsUsed || result.metrics?.agentCount || 0,
