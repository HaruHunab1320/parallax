...
export class ThreadPersistenceService {
  ...
  async projectRuntimeEvent(payload: RuntimeThreadProjection): Promise<void> {
    await this.repository.recordRuntimeProjection(payload.thread, payload.event, payload.runtime);
  }

  async recoverFromRuntimes(agentRuntimeService: AgentRuntimeService): Promise<number> {
    ...
    this.logger.info({ recoveredThreads: threads.length }, 'Recovered runtime-backed threads into persistence');
