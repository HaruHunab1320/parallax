...
export function createPatternsRouter(
  ...
  const requirePatternManagement = (
    _req: Request,
    res: Response,
    next: NextFunction
  ): void => {
  ...
  router.post('/:name/validate', async (req: any, res: any) => {
    ...
    try {
      ...
      return res.status(500).json({
        error:
          error instanceof Error ? error.message : 'Failed to validate pattern',
      });
    }
  });
    ...
    try {
      ...
      if (database) {
        try {
          ...
          logger.warn(
            { error: dbError },
            'Failed to create execution record in database'
          );
        }
      }
      ...
      if (database && dbExecutionId) {
        try {
          await updateExecutionInDb(database, dbExecutionId, {
            ...
            confidence:
              result.metrics?.averageConfidence ?? result.confidence ?? 0,
            durationMs: duration,
          ...
          logger.warn(
            { error: dbError },
            'Failed to update execution record in database'
          );
        }
      }
      ...
      if (database && dbExecutionId) {
        try {
          ...
          logger.warn(
            { error: dbError },
            'Failed to update failed execution in database'
          );
        }
      }
      ...
      return res.status(500).json({
        error:
          error instanceof Error ? error.message : 'Failed to execute pattern',
      });
    }
  });
  ...
  router.get('/:name/metrics', async (_req: any, res: any) => {
    ...
    try {
      ...
      const patternMetrics = allMetrics.filter(
        (m) => m.pattern === name || m.patternName === name
      );
      ...
      const stats = {
        ...
        avgDuration:
          patternMetrics.reduce((sum, m) => sum + (m.duration || 0), 0) /
          patternMetrics.length,
        avgConfidence:
          patternMetrics.reduce((sum, m) => sum + (m.confidence || 0), 0) /
          patternMetrics.length,
        successRate:
          patternMetrics.filter((m) => m.success).length /
          patternMetrics.length,
      ...
      logger.error(
        { error, patternName: name },
        'Failed to get pattern metrics'
      );
      return res.status(500).json({
  ...
  router.put('/:name', requirePatternManagement, async (req: any, res: any) => {
    ...
    try {
      ...
      if (
        patternWithSource.source === 'file' &&
        !patternEngine.hasDatabasePatterns()
      ) {
        metrics.recordApiCall('patterns', 'update', 403);
        return res.status(403).json({
          error:
            'Cannot update file-based patterns. Edit the .prism file directly or enable database storage.',
        });
      }
  ...
  router.post(
    ...
    async (req: any, res: any) => {
      ...
      try {
        const result = await parseAndSavePattern(
          filename,
          content,
          overwrite ?? false
        );
        ...
        logger.info(
          { filename, patternName: result.pattern.name },
          'Pattern uploaded via API'
        );
        return res.status(201).json(result);
      } catch (error) {
        const message =
          error instanceof Error ? error.message : 'Failed to upload pattern';
  ...
  router.post(
    ...
    async (req: any, res: any) => {
      ...
      logger.info(
        { total: files.length, success: successCount },
        'Batch pattern upload completed'
      );
  ...
  async function parseAndSavePattern(
    ...
    if (ext === '.yaml' || ext === '.yml') {
      ...
      const pattern = await patternEngine.savePattern(patternData, {
        overwrite,
      });
    ...
    throw new Error(
      `Unsupported file extension: ${ext}. Use .prism, .yaml, or .yml`
    );
  }
  ...
  router.get(
    ...
    async (req: any, res: any) => {
      ...
      try {
        ...
        logger.error(
          { error, patternName: name },
          'Failed to get pattern versions'
        );
        return res.status(500).json({
          error:
