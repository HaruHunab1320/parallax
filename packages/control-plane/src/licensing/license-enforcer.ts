...
export class LicenseEnforcer {
  ...
  private validateLicenseKey(key: string): LicenseInfo {
    ...
    if (
      key.startsWith('PARALLAX-ENT-') ||
      key.startsWith('PARALLAX-PLUS-') ||
  ...
  private validateSignedLicenseKey(key: string): LicenseInfo {
    ...
    const publicKeyOverride =
      process.env.PARALLAX_LICENSE_PUBLIC_KEY || undefined;
    ...
    if (payload.tier === 'enterprise-plus') {
      return {
        ...
        validUntil:
          payload.exp === 0 ? undefined : new Date(payload.exp * 1000),
        clusterId: payload.cluster,
        features: [...OPENSOURCE_FEATURES, ...ENTERPRISE_ONLY_FEATURES],
      };
    }
    ...
    return {
      ...
      features: [
        ...OPENSOURCE_FEATURES,
        ...ENTERPRISE_ONLY_FEATURES.filter(
          (f) => !enterprisePlusOnly.includes(f)
        ),
      ],
    };
  }
...
╚══════════════════════════════════════════════════════════════════╝`;
  ...
  private getEnterpriseBanner(): string {
    const validUntil =
      this.license.validUntil?.toLocaleDateString() || 'Perpetual';
    const tier =
      this.license.type === 'enterprise-plus'
        ? 'Enterprise Plus'
        : 'Enterprise';
    const support =
      this.license.type === 'enterprise-plus'
  ...
  trackFeatureAccess(feature: string, granted: boolean): void {
    this.logger.debug(
      ...
      granted
        ...
        : 'Feature access denied (upgrade required)'
    );
  }
}
