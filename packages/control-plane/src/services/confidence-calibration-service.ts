...
export class ConfidenceCalibrationService {
  ...
  calibrateResponse<T>(
    ...
    if (
      !performance ||
      performance.totalPredictions < this.getMinSamples(domain)
    ) {
    ...
    const calibratedConfidence = Math.max(
      0,
      Math.min(1, response.confidence + calibrationAdjustment)
    );
  ...
  calibrateResponses<T>(
    ...
    return responses.map((response) =>
      this.calibrateResponse(response, domain)
    );
  }
  ...
  async recordOutcome(
    ...
    const config =
      this.calibrationConfigs.get(domain) ||
      this.calibrationConfigs.get('general')!;

    // Apply decay to old predictions
    performance.totalPredictions =
      performance.totalPredictions * config.decayFactor + 1;
    performance.correctPredictions =
      performance.correctPredictions * config.decayFactor +
      (wasCorrect ? 1 : 0);

    // Update running averages
    performance.averageConfidence =
      (performance.averageConfidence * (performance.totalPredictions - 1) +
        predictedConfidence) /
      performance.totalPredictions;

    performance.actualAccuracy =
      performance.correctPredictions / performance.totalPredictions;
    performance.calibrationError =
      performance.averageConfidence - performance.actualAccuracy;
  ...
  private calculateAdjustment(
    ...
    const config =
      this.calibrationConfigs.get(performance.domain) ||
      this.calibrationConfigs.get('general')!;
    ...
    const confidenceScale =
      confidence > 0.8 ? 1.2 : confidence > 0.5 ? 1.0 : 0.8;
  ...
  importCalibrationData(jsonData: string): void {
    try {
      ...
      if (data.performanceData && Array.isArray(data.performanceData)) {
        ...
        data.performanceData.forEach(
          ([key, value]: [string, AgentPerformance]) => {
