...
agentCommand
  ...
  .action(async (agentId) => {
    ...
    try {
      ...
      console.log(chalk.gray('─'.repeat(50)));
      console.log(
        chalk.white('Value: ') + JSON.stringify(result.value, null, 2)
      );
      console.log(
        chalk.white('Confidence: ') + chalk.yellow(result.confidence.toFixed(2))
      );
...
agentCommand
  ...
  .action(async () => {
    ...
    try {
      ...
      const capabilities = Array.from(capabilityMap.entries()).sort(
        (a, b) => b[1] - a[1]
      );
