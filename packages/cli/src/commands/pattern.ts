...
export const patternCommand = new Command('pattern').description(
  'Manage coordination patterns'
);
    ...
    try {
      ...
      if (options.verbose) {
        patterns.forEach((pattern) => {
          ...
          console.log(
            chalk.white('Min Agents: ') + (pattern.minAgents || 'Any')
          );
...
patternCommand
  ...
  .action(async (filePath, options) => {
    ...
    try {
      ...
      const client = new ParallaxHttpClient();
      const pattern = await client.uploadPattern(
        filename,
        content,
        options.overwrite
      );
      ...
      spinner.succeed(
        chalk.green(`Pattern "${pattern.name}" uploaded successfully`)
      );
...
patternCommand
  ...
  .action(async (dir, options) => {
    ...
    try {
      ...
      const client = new ParallaxHttpClient();
      const { results } = await client.uploadPatterns(
        fileContents,
        options.overwrite
      );
  ...
  .action(async (patternName) => {
    ...
    try {
      ...
      if (pattern.minAgents)
        console.log(chalk.white('Min Agents: ') + pattern.minAgents);
      if (pattern.maxAgents)
        console.log(chalk.white('Max Agents: ') + pattern.maxAgents);
