  ...
  .action(async (options) => {
    ...
    try {
      ...
      try {
        ...
        spinner.fail(
          chalk.red(
            `Patterns directory not found: ${process.env.PARALLAX_PATTERNS_DIR}`
          )
        );
        console.log(
          chalk.yellow(
            'Create it or specify a different directory with --patterns'
          )
        );
        process.exit(1);
      }
      ...
      console.log(
        chalk.gray('  Control Plane API: ') +
          chalk.white(`http://localhost:${options.port}`)
      );
      console.log(
        chalk.gray('  Health Check:      ') +
          chalk.white(`http://localhost:${options.port}/health`)
      );
      console.log(
        chalk.gray('  Patterns:          ') +
          chalk.white(`http://localhost:${options.port}/api/patterns`)
      );
      console.log(
        chalk.gray('  Agents:            ') +
          chalk.white(`http://localhost:${options.port}/api/agents`)
      );

      console.log(`\n${chalk.cyan('Quick Start:')}`);
      console.log(
        chalk.gray('  1. Start an agent:     ') +
          chalk.white('parallax agent start <agent-path>')
      );
      console.log(
        chalk.gray('  2. List patterns:      ') +
          chalk.white('parallax pattern list')
      );
      console.log(
        chalk.gray('  3. Execute a pattern:  ') +
          chalk.white('parallax run <pattern-name>')
      );
