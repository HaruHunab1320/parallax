    ...
    try {
      ...
      const status = {
        ...
        metrics: {
          ...
          capabilities: [
            ...new Set(agents.flatMap((a: AgentInfo) => a.capabilities)),
          ].length,
        },
      };
      ...
      console.log(
        chalk.white('Version: ') + chalk.cyan(status.platform.version)
      );
      console.log(
        chalk.white('Status: ') +
          (status.platform.status === 'healthy'
            ? chalk.green(status.platform.status)
            : chalk.yellow(status.platform.status))
      );

      // Services Status
      console.log(`\n${chalk.cyan('Services:')}`);

      const serviceTableData: any[] = [['Component', 'Status', 'Health']];

      Object.entries(status.services.controlPlane).forEach(
        ([name, info]: [string, any]) => {
          serviceTableData.push([
            name,
            info.status === 'online' ? chalk.green('●') : chalk.red('●'),
            info.health === 'healthy'
              ? chalk.green(info.health)
              : info.health === 'degraded'
                ? chalk.yellow(info.health)
      ...
      console.log(chalk.gray('─'.repeat(50)));
      console.log(
        chalk.white('Total Registered: ') + status.services.agents.total
      );
      console.log(
        chalk.white('Available: ') +
          chalk.green(status.services.agents.available.toString())
      );
      console.log(
        chalk.white('Offline: ') +
          (status.services.agents.offline > 0
            ? chalk.red(status.services.agents.offline.toString())
            : '0')
      );
      ...
      console.log(chalk.gray('─'.repeat(50)));
      console.log(
        chalk.white('Patterns Available: ') + status.metrics.patternsLoaded
      );
      console.log(
        chalk.white('Unique Capabilities: ') + status.metrics.capabilities
      );

      // Pattern List
      if (patterns.length > 0 && patterns.length <= 5) {
        console.log(
          chalk.white('Patterns: ') + chalk.gray(patterns.join(', '))
        );
      }
      ...
      if (status.services.agents.available === 0) {
        ...
        console.log(
          chalk.gray(
            '  Start agents or set PARALLAX_LOCAL_AGENTS environment variable'
          )
        );
