/**
 * Kubernetes Runtime HTTP Server
 *
...
export class RuntimeServer {
  ...
  private setupRoutes(): void {
    ...
    router.post('/agents/:id/restart', async (req: Request, res: Response) => {
      try {
        const agent = await this.runtime.restart(req.params.id);
        res.json(agent);
      } catch (error) {
        this.handleError(res, error);
      }
    ...
    this.app.get('/', (_req: Request, res: Response) => {
      res.json({
        service: 'Parallax Kubernetes Runtime',
        version: '0.1.0',
        endpoints: {
          health: '/api/health',
          agents: '/api/agents',
  ...
  async start(): Promise<void> {
    ...
    return new Promise((resolve) => {
      this.server!.listen(port, host, () => {
        this.logger.info({ port, host }, 'K8s runtime server listening');
        resolve();
      });
    });
  }
