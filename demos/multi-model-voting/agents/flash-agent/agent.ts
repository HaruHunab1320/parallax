/**
 * Gemini Flash Lite Voting Agent
 *
 * Uses Google's lightweight Flash Lite model for efficient decision-making.
...
const AGENT_ID = 'gemini-flash';
const AGENT_NAME = 'Gemini 3 Flash Voter';

class FlashAgent extends ParallaxAgent {
  ...
  constructor() {
    super(
      ...
      ['voting', 'classification', 'decision'],
      {
        expertise: 0.80,
        model: MODEL_NAME,
        description: 'Balanced decision-making with Gemini 3 Flash'
      }
    );
...
async function main() {
  const agent = new FlashAgent();
  const port = parseInt(process.env.AGENT_PORT || '50202', 10);
