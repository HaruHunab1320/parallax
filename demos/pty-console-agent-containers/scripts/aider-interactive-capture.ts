...
function parseArgs(argv: string[]): CliArgs {
  ...
  const out: CliArgs = {
    ...
    outputDir: resolve(
      process.env.AIDER_CAPTURE_OUTPUT_DIR ?? '.parallax/pty-captures'
    ),
    cols: Number.parseInt(
      process.env.AIDER_CAPTURE_COLS ?? String(stdoutCols),
      10
    ),
    rows: Number.parseInt(
      process.env.AIDER_CAPTURE_ROWS ?? String(stdoutRows),
      10
    ),
  };
...
async function main(): Promise<void> {
  ...
  const teardown = async (reason: string): Promise<void> => {
    ...
    if (snapshot) {
      process.stderr.write(
        `[aider-capture] raw: ${snapshot.paths.rawEventsPath}\n`
      );
      process.stderr.write(
        `[aider-capture] states: ${snapshot.paths.statesPath}\n`
      );
      process.stderr.write(
        `[aider-capture] transitions: ${snapshot.paths.transitionsPath}\n`
      );
      process.stderr.write(
        `[aider-capture] lifecycle: ${snapshot.paths.lifecyclePath}\n`
      );
      process.stderr.write(
        `[aider-capture] final-state: ${snapshot.state.state} (${snapshot.state.ruleId ?? 'no-rule'})\n`
      );
    }
  };
  ...
  process.stderr.write(
    `[aider-capture] auto-respond=${autoRespond ? 'on' : 'off'}\n`
  );
