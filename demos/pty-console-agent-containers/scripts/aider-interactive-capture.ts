...
function parseArgs(argv: string[]): CliArgs {
  ...
  const out: CliArgs = {
    workdir: resolve(process.env.AIDER_CAPTURE_WORKDIR ?? process.cwd()),
    outputDir: resolve(process.env.AIDER_CAPTURE_OUTPUT_DIR ?? '.parallax/pty-captures'),
    cols: Number.parseInt(process.env.AIDER_CAPTURE_COLS ?? String(stdoutCols), 10),
    rows: Number.parseInt(process.env.AIDER_CAPTURE_ROWS ?? String(stdoutRows), 10),
  };
...
function printHelp(): void {
  console.log(`
Aider Interactive Capture

Usage:
  tsx scripts/aider-interactive-capture.ts [options]
...
Exit:
  - Type /exit in Aider, or
  - Press Ctrl+] in this wrapper

Env:
  AIDER_CAPTURE_WORKDIR
  AIDER_CAPTURE_OUTPUT_DIR
  AIDER_CAPTURE_COLS
  AIDER_CAPTURE_ROWS
  `);
}
...
async function main(): Promise<void> {
  ...
  const runId = `aider-interactive-${new Date().toISOString().replace(/[:.]/g, '-')}-${randomUUID().slice(0, 8)}`;
  ...
  const teardown = async (reason: string): Promise<void> => {
    ...
    process.stderr.write(`\n[aider-capture] stopped (${reason})\n`);
    if (snapshot) {
      process.stderr.write(`[aider-capture] raw: ${snapshot.paths.rawEventsPath}\n`);
      process.stderr.write(`[aider-capture] states: ${snapshot.paths.statesPath}\n`);
      process.stderr.write(`[aider-capture] transitions: ${snapshot.paths.transitionsPath}\n`);
      process.stderr.write(`[aider-capture] lifecycle: ${snapshot.paths.lifecyclePath}\n`);
      process.stderr.write(`[aider-capture] final-state: ${snapshot.state.state} (${snapshot.state.ruleId ?? 'no-rule'})\n`);
    }
  };
  ...
  process.stderr.write(`[aider-capture] session=${session.id}\n`);
  process.stderr.write(`[aider-capture] run-id=${runId}\n`);
  process.stderr.write(`[aider-capture] auto-respond=${autoRespond ? 'on' : 'off'}\n`);
  process.stderr.write('[aider-capture] press Ctrl+] to detach\n\n');
...
main().catch((error) => {
  console.error('aider-interactive-capture failed:', error);
  process.exitCode = 1;
});
