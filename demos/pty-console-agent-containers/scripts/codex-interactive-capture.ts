...
function parseArgs(argv: string[]): CliArgs {
  ...
  const out: CliArgs = {
    ...
    outputDir: resolve(
      process.env.CODEX_CAPTURE_OUTPUT_DIR ?? '.parallax/pty-captures'
    ),
    cols: Number.parseInt(
      process.env.CODEX_CAPTURE_COLS ?? String(stdoutCols),
      10
    ),
    rows: Number.parseInt(
      process.env.CODEX_CAPTURE_ROWS ?? String(stdoutRows),
      10
    ),
  };
...
async function main(): Promise<void> {
  ...
  const teardown = async (reason: string): Promise<void> => {
    ...
    if (snapshot) {
      process.stderr.write(
        `[codex-capture] raw: ${snapshot.paths.rawEventsPath}\n`
      );
      process.stderr.write(
        `[codex-capture] states: ${snapshot.paths.statesPath}\n`
      );
      process.stderr.write(
        `[codex-capture] transitions: ${snapshot.paths.transitionsPath}\n`
      );
      process.stderr.write(
        `[codex-capture] lifecycle: ${snapshot.paths.lifecyclePath}\n`
      );
      process.stderr.write(
        `[codex-capture] final-state: ${snapshot.state.state} (${snapshot.state.ruleId ?? 'no-rule'})\n`
      );
    }
  };
  ...
  process.stderr.write(
    `[codex-capture] auto-respond=${autoRespond ? 'on' : 'off'}\n`
  );
