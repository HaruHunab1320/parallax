...
import { CodexAdapter } from '../../../packages/coding-agent-adapters/src/codex-adapter.ts';
...
function parseArgs(argv: string[]): CliArgs {
  ...
  const out: CliArgs = {
    workdir: resolve(process.env.CODEX_CAPTURE_WORKDIR ?? process.cwd()),
    outputDir: resolve(process.env.CODEX_CAPTURE_OUTPUT_DIR ?? '.parallax/pty-captures'),
    cols: Number.parseInt(process.env.CODEX_CAPTURE_COLS ?? String(stdoutCols), 10),
    rows: Number.parseInt(process.env.CODEX_CAPTURE_ROWS ?? String(stdoutRows), 10),
  };
...
function printHelp(): void {
  console.log(`
Codex Interactive Capture

Usage:
  tsx scripts/codex-interactive-capture.ts [options]
...
Exit:
  - Type /exit in Codex, or
  - Press Ctrl+] in this wrapper

Env:
  CODEX_CAPTURE_WORKDIR
  CODEX_CAPTURE_OUTPUT_DIR
  CODEX_CAPTURE_COLS
  CODEX_CAPTURE_ROWS
  `);
}
...
async function main(): Promise<void> {
  ...
  const runId = `codex-interactive-${new Date().toISOString().replace(/[:.]/g, '-')}-${randomUUID().slice(0, 8)}`;
  ...
  const teardown = async (reason: string): Promise<void> => {
    ...
    process.stderr.write(`\n[codex-capture] stopped (${reason})\n`);
    if (snapshot) {
      process.stderr.write(`[codex-capture] raw: ${snapshot.paths.rawEventsPath}\n`);
      process.stderr.write(`[codex-capture] states: ${snapshot.paths.statesPath}\n`);
      process.stderr.write(`[codex-capture] transitions: ${snapshot.paths.transitionsPath}\n`);
      process.stderr.write(`[codex-capture] lifecycle: ${snapshot.paths.lifecyclePath}\n`);
      process.stderr.write(`[codex-capture] final-state: ${snapshot.state.state} (${snapshot.state.ruleId ?? 'no-rule'})\n`);
    }
  };
...
main().catch((error) => {
  console.error('codex-interactive-capture failed:', error);
  process.exitCode = 1;
});
